<?php
/*
Plugin Name: WO2 ObviousNameLitmus
...
if (class_exists('Litmus')) {
	add_action('wo3_prep', array('ObviousNameLitmus', 'getCount'));
	add_action('wo3_tabs', array('ObviousNameLitmus', 'tab'));
	add_action('wo3_content', array('ObviousNameLitmus', 'content'));
	add_action('wo3_add_index', array('ObviousNameLitmus', 'addIndex'));

	class ObviousNameLitmus extends Litmus {
		...
		public static function getMatches() {
			...
			return self::runCachedMatchesQuery(
				self::getName(),
				"SELECT comment_author, comment_id, MATCH (comment_author) AGAINST ('".$obvious_words."' IN BOOLEAN MODE) as score FROM wp_comments where comment_approved='spam' and MATCH (comment_author) AGAINST ('".$obvious_words."' IN BOOLEAN MODE) order by score desc;"
			);
		}
		...
		function prepare($comments) {
			...
			foreach($comments as $item) {
				$num=0;
				$new_name_bits = array();
				$name_bits = explode(" ",strtolower($item->comment_author));
				...
				$item->comment_author = implode(" ",$new_name_bits);
				$item->num = $num;
				$results[] = $item;
			}
			return $results;
